...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    n = len(IDandRatingsTuple[1])
    return (IDandRatingsTuple[0] , (n , float(sum(IDandRatingsTuple[1]))/n))
...
movieIDsWithRatingsRDD = (ratingsRDD.map(lambda s: (s[1], [s[2]]) ) ).reduceByKey(lambda x ,y: x + y)
...
movieNameWithAvgRatingsRDD = (moviesRDD.join(movieIDsWithAvgRatingsRDD).map(lambda s: (s[1][1][1] , s[1][0] , s[1][1][0])))
...
movieLimitedAndSortedByRatingRDD = (movieNameWithAvgRatingsRDD
                                    .filter(lambda s: True if s[2] > 500 else False)
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda s: math.pow(s[1][0] - s[1][1],2)))
...
# TODO: Replace <FILL IN> with appropriate code
myModel =ALS.train(trainingRDD, bestRank, seed = 5L, iterations = 5,lambda_ = 0.1)
...
myRatedMovies = [
     (myUserID, 1, 5.0) , (myUserID, 2, 2.0) , (myUserID, 3, 4.0) , (myUserID, 4, 5.0) , (myUserID, 5, 1.0) ,
    (myUserID, 6, 2.0) , (myUserID, 7, 5.0) , (myUserID, 8, 3.0) , (myUserID, 9, 5.0) , (myUserID, 10, 4.0) 
...
# TODO: Replace <FILL IN> with appropriate code
myRatingsModel = ALS.train(trainingWithMyRatingsRDD, bestRank, seed = 5L, iterations = 5,lambda_ = 0.1)
...
myUnratedMoviesRDD = moviesRDD.map(lambda s: (0 , s[0]))
...
predictedWithCountsRDD  = (predictedRDD
                           .join(movieCountsRDD))
                           .map(lambda s: (s[0] , (s[1][0], s[1][1][0]) ) ))
print predictedWithCountsRDD.take(3)
