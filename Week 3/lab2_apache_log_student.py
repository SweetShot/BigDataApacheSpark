...
endpointCountPairTuple = not200.map(lambda s: (s.endpoint ,1))
...
badRecords = (access_logs.filter(lambda s: s.response_code == 404)).cache()
...
errDateCountPairTuple = badRecords.map(lambda s: (s.date_time.day ,1))
