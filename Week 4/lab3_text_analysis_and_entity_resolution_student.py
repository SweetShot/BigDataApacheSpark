...
def tokenize(string):
    ...
    """
    return filter( None , filter( lambda str: '' if str in stopwords else str , re.split(split_regex, string.lower()) ) )
...
def countTokens(vendorRDD):
    ...
    """
    return len(vendorRDD.map(lambda s: s[1]).reduce(lambda a, b: a + b))
...
def cossim(a, b):
    ...
    """
    return float(dotprod(a,b))/(norm(a)*norm(b))
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda s: (s[1] +" " +s[0] , s[2]))

trueDupsRDD = (sims.join(goldStandard)).map(lambda s: (s[0] , s[1][0]))
trueDupsCount = trueDupsRDD.count()
avgSimDups = sum(trueDupsRDD.map(lambda s: s[1]).collect())/trueDupsRDD.count()

nonDupsRDD = (sims.subtractByKey(trueDupsRDD))
avgSimNon = sum(nonDupsRDD.map(lambda s: s[1]).collect())/nonDupsRDD.count()
...
amazonWeightsRDD = amazonFullRecToToken.map(lambda s: (s[0] , tfidf(s[1] , idfsFullBroadcast.value )) )
googleWeightsRDD = googleFullRecToToken.map(lambda s: (s[0] , tfidf(s[1] , idfsFullBroadcast.value)) )
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map(lambda s: (s[0],norm(s[1])))
amazonNormsBroadcast = sc.broadcast(amazonNorms.collectAsMap())
googleNorms = googleWeightsRDD.map(lambda s: (s[0],norm(s[1])))
...
def invert(record):
    ...
    """
    return [[k,record[0]] for k in record[1]]
...
commonTokens = (amazonInvPairsRDD.join(googleInvPairsRDD).map(swap).reduceByKey(lambda x ,y: x + y )).cache()
